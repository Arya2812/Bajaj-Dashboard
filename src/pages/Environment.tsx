...
const CLUSTER_COLORS = ["#0052A3", "#2A7ADE", "#22C55E", "#8B5CF6", "#F59E0B", "#EF4444", "#06B6D4", "#84CC16"];
...
const PILL_CLS = "text-xs font-medium bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-slate cursor-pointer outline-none hover:border-cobalt transition-colors appearance-none pr-7";

function SelectPill({ value, onChange, children }: { value: string; onChange: (v: string) => void; children: React.ReactNode }) {
  return (
    <div className="relative inline-flex items-center">
      <select value={value} onChange={e => onChange(e.target.value)} className={PILL_CLS}>{children}</select>
              ...
              <div key={name}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[11px] font-bold ml-2 flex-shrink-0" style={{ color }}>
                    {count}<span className="text-slate-light font-normal ml-1">({pct.toFixed(0)}%)</span>
                  </span>
                </div>
...
export default function Environment() {
  ...
  const avgPotential = envR.length ? envR.reduce((s, r) => s + (parseFloat(r.bajaj_overall_potential) || 0), 0) / envR.length : 0;
  ...
  const clusterData = Object.entries(clusterMap)
    .map(([fullName, { count, potSum }]) => {
      ...
      return { name: firstSegment.length > 32 ? firstSegment.slice(0, 30) + "…" : firstSegment, fullName, count, avgPotential: Math.round((potSum / count) * 10) / 10 };
    })
  ...
  envR.forEach(r => {
    const s = computeEnvDimensionScores({
      primary_orientation: r.primary_orientation, secondary_orientation: r.secondary_orientation,
      store_format: r.store_format, display_structure: r.display_structure,
      customer_type: r.customer_type, selling_behavior: r.selling_behavior,
      store_size_band: r.store_size_band, brand_presence: r.brand_presence,
      category_depth: r.category_depth, catchment_power: r.catchment_power,
    });
    if (s) DIM_CONFIG.forEach(d => { dimSums[d.key] += s[d.key as keyof typeof s]; dimCounts[d.key] += 1; });
  });

  const topTier   = topEntry(tierFreq);
  const hasFilters = !!(filterZone || filterBand || filterCategory);
  const potColor   = avgPotential >= 65 ? "#22C55E" : avgPotential >= 50 ? "#F59E0B" : "#EF4444";

  return (
    <div className="flex flex-col min-h-screen" style={{ background: "#F0F2F5" }}>
      <TopBar title="Environment Intelligence" count={data.length} loading={loading} onRefresh={fetchData} search={search} onSearch={setSearch} />
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-2.5 flex items-center gap-3 flex-wrap">
        ...
        {hasFilters && (
          <button onClick={() => { setFilterZone(""); setFilterBand(""); setFilterCategory(""); }} className="text-xs font-semibold text-cobalt hover:underline ml-1">Reset</button>
        )}
      </div>

      <div className="flex-1 px-6 py-6 space-y-5">
        {error && <div className="bg-red-50 border border-red-200 text-red-600 text-sm rounded-xl p-4">{error}</div>}
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              <KpiCard title="Analyzed Outlets"    value={envR.length}                accent="#0052A3" />
              <KpiCard title="Avg Outlet Potential" value={`${avgPotential.toFixed(1)}%`} accent={potColor} sub={topEntry(tierFreq)} />
              <KpiCard title="Top Potential Tier"  value={topTier.split(" ")[0]}       accent={TIER_COLORS[topTier] ?? "#2A7ADE"} sub={`${tierFreq[topTier] ?? 0} outlets`} />
              <KpiCard title="Mapped Rule %"       value={`${mappedPct.toFixed(0)}%`}  accent="#8B5CF6" sub={`${mappedCount} of ${envR.length} outlets`} />
              <KpiCard title="Top Cluster"         value={clusterData[0]?.count ?? 0} accent="#22C55E" sub={clusterData[0]?.name ?? "—"} />
            </div>
            ...
            <div className="bg-white rounded-2xl p-5" style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[11px] text-slate-light bg-[#F0F4F8] px-2 py-0.5 rounded-full">{Object.keys(clusterMap).length} unique clusters</span>
              </div>
              {clusterData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={Math.max(180, clusterData.length * 36)}>
                  <BarChart data={clusterData} layout="vertical" margin={{ top: 0, right: 48, left: 4, bottom: 0 }}>
                    <XAxis type="number" tick={{ fontSize: 10, fill: "#6B7A84" }} axisLine={false} tickLine={false} allowDecimals={false} />
                    <YAxis type="category" dataKey="name" tick={{ fontSize: 10, fill: "#3A4750" }} width={180} axisLine={false} tickLine={false} />
                    <Tooltip contentStyle={{ borderRadius: 10, border: "none", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", fontSize: 12 }}
                      formatter={(val: number, name: string) => [val, name === "count" ? "Outlets" : "Avg Potential %"]}
                      labelFormatter={(label: string, payload) => { const full = payload?.[0]?.payload?.fullName; return full ?? label; }} />
                    <Bar dataKey="count" radius={[0, 6, 6, 0]} maxBarSize={20}>
                      {clusterData.map((_, i) => <Cell key={i} fill={CLUSTER_COLORS[i % CLUSTER_COLORS.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      ...
                      <div key={tier}>
                        <div className="flex items-center justify-between mb-1.5">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-bold w-9 text-right" style={{ color }}>{pct.toFixed(0)}%</span>
                          </div>
                        </div>
                ...
                <div className="mt-5 pt-4 border-t border-[#F0F4F8]">
                  ...
                  <div className="flex gap-3">
                    {Object.entries(ruleFreq).map(([src, cnt]) => (
                      <div key={src} className="flex-1 rounded-xl p-3 text-center" style={{ background: "#F8FAFC", border: "1px solid #EEF2F7" }}>
                        <p className="text-xl font-bold text-slate">{cnt}</p>
                        <p className="text-[10px] text-slate-light mt-0.5 leading-tight">{src}</p>
                      </div>
                    ))}
                    {Object.keys(ruleFreq).length === 0 && <p className="text-xs text-slate-light">No data</p>}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl p-5" style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
                <div className="flex items-center justify-between mb-4">
                  <p className="text-sm font-semibold text-slate">Avg Dimension Scores <span className="text-slate-light font-normal text-xs ml-1">(out of 5.0)</span></p>
                  <span className="text-[11px] text-slate-light bg-[#F0F4F8] px-2 py-0.5 rounded-full">{dimCounts[DIM_CONFIG[0].key] ?? 0} outlets</span>
                </div>
                  ...
                  {DIM_CONFIG.map(dim => {
                    ...
                    return (
                      <div key={dim.key}>
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-2">
                            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md flex-shrink-0" style={{ background: `${dim.color}18`, color: dim.color }}>{dim.weight}%</span>
                            <span className="text-xs font-medium text-slate">{dim.label}</span>
                          </div>
                          <span className="text-xs font-bold flex-shrink-0 ml-2" style={{ color: dim.color }}>{avg.toFixed(2)}</span>
                        </div>
                        <div className="h-2 rounded-full overflow-hidden bg-[#F0F4F8]">
                          <div className="h-full rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: `linear-gradient(90deg, ${dim.color}, ${dim.color}88)` }} />
                        </div>
                      </div>
                    );
                  })}
