...
export default function Retailers() {
  ...
  const downloadCsv = () => {
    ...
    const rows = filtered.map(r => [
      r.retailer_name, r.city, r.zone,
      r.fmr_final_pct ? `${(parseFloat(r.fmr_final_pct) * 100).toFixed(1)}%` : "—",
      r.fmr_score_band, r.final_environment_cluster,
      r.bajaj_potential_tier, r.fmr_opportunity_type, formatDate(r.timestamp),
    ]);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#EBEFF5]" style={{ marginRight: selected ? 440 : 0, transition: "margin-right 0.3s ease" }}>
      <TopBar title="All Retailers" count={data.length} loading={loading} onRefresh={fetchData} search={search} onSearch={setSearch} />
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-2.5 flex items-center gap-3 flex-wrap">
        ...
        {[
          { value: filterCity, onChange: setFilterCity, placeholder: "All Cities", options: allCities },
          { value: filterZone, onChange: setFilterZone, placeholder: "All Zones",  options: ZONES },
          { value: filterBand, onChange: setFilterBand, placeholder: "All Bands",  options: BANDS },
        ].map(({ value, onChange, placeholder, options }) => (
          <div key={placeholder} className="relative inline-flex items-center">
        ...
        {hasFilters && (
          <button onClick={() => { setFilterCity(""); setFilterZone(""); setFilterBand(""); }} className="text-xs text-cobalt hover:underline font-medium ml-1">Reset</button>
        )}
        <div className="flex-1" />
        <button onClick={downloadCsv} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-green-600 hover:bg-green-700 text-white transition-colors">
      ...
      <div className="mx-6 mt-4 mb-6 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-5 py-3.5 border-b border-gray-100 flex items-center gap-3">
          ...
          <span className="text-xs font-semibold text-slate-light bg-gray-100 rounded-full px-2.5 py-0.5">{filtered.length} entries</span>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {COLS.map(col => (
                    <th key={col.key} className="px-4 py-3 text-left cursor-pointer select-none hover:bg-gray-100 transition-colors" onClick={() => handleSort(col.key)}>
                      <div className="flex items-center gap-1 text-[11px] font-semibold uppercase tracking-wide text-slate-light">
                        {col.label}
                        {sortKey === col.key
                          ? (sortDir === "asc" ? <ChevronUp size={11} className="text-cobalt" /> : <ChevronDown size={11} className="text-cobalt" />)
              ...
              <tbody>
                {filtered.map((r, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => setSelected(r)}>
                      ...
                      <td className="px-4 py-3"><span className="text-xs px-2 py-0.5 rounded-full bg-[#EBEFF5] text-slate font-medium">{r.zone || "—"}</span></td>
                      ...
                      <td className="px-4 py-3"><span className="text-xs font-bold px-2 py-1 rounded-full text-white" style={{ background: color }}>{band || "—"}</span></td>
                      ...
                      <td className="px-4 py-3"><span className="text-xs font-bold text-cobalt">{r.fmr_opportunity_type || "—"}</span></td>
                      <td className="px-4 py-3 text-xs text-slate-light whitespace-nowrap">{formatDate(r.timestamp)}</td>
                    </tr>
                  );
                })}
                {filtered.length === 0 && (
                  <tr><td colSpan={COLS.length} className="px-4 py-12 text-center text-slate-light">
      ...
      {selected && <RetailerInsightsDrawer retailer={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}
