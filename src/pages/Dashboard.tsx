...
function SelectPill({ value, onChange, children }: { value: string; onChange: (v: string) => void; children: React.ReactNode }) {
  return (
    <div className="relative inline-flex items-center">
      <select value={value} onChange={e => onChange(e.target.value)} className={PILL_SELECT}>{children}</select>
...
export default function Dashboard() {
  ...
  const avgScore = filtered.length ? filtered.reduce((s, r) => s + (parseFloat(r.fmr_final_pct) || 0), 0) / filtered.length : 0;
  ...
  const bandData = BANDS.map(b => ({ name: b, value: filtered.filter(r => r.fmr_score_band === b).length })).filter(d => d.value > 0);
  const scoreDist = SCORE_RANGES.map(sr => ({
    range: sr.label,
    count: filtered.filter(r => { const p = parseFloat(r.fmr_final_pct) || 0; return p >= sr.min && p < sr.max; }).length,
  }));
  const topRetailers = [...filtered].sort((a, b) => (parseFloat(b.fmr_final_pct) || 0) - (parseFloat(a.fmr_final_pct) || 0));
  const cityBarData = Object.entries(cityFreq).sort((a, b) => b[1] - a[1]).map(([city, count]) => ({ city, count }));
  const zoneData = ZONES.map(zone => {
    const zr  = filtered.filter(r => r.zone === zone);
    const avg = zr.length ? zr.reduce((s, r) => s + (parseFloat(r.fmr_final_pct) || 0), 0) / zr.length : 0;
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ background: "#F0F2F5" }}>
      <TopBar title="Overview" count={data.length} loading={loading} onRefresh={fetchData} search={search} onSearch={setSearch} />
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-2.5 flex items-center gap-3 flex-wrap">
        ...
        {hasFilters && (
          <button onClick={() => { setFilterZone(""); setFilterBand(""); setFilterCategory(""); }} className="text-xs font-semibold text-cobalt hover:underline ml-1">
            Reset
          </button>
        )}
      </div>
