...
export default function Form() {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8">
        <div className="neu-card p-10 max-w-md w-full text-center">
          ...
          <h2 className="text-2xl font-bold text-slate mb-2">{demoMode ? "Scores Computed!" : "Retailer Saved!"}</h2>
          ...
          <div className="flex gap-3 justify-center">
            <button className="neu-btn px-6 py-2.5 text-sm" onClick={() => navigate("/dashboard")}>View Dashboard</button>
            <button className="neu-btn-outline px-6 py-2.5 text-sm" onClick={() => { setForm(initForm()); setSubmitted(false); setStep(0); }}>Add Another</button>
          </div>
        </div>
      </div>
    );
  ...
  const dimScoreList = dimScores ? FMR_DIMENSIONS.map(d => ({ id: d.id, label: d.label, score: dimScores[d.id] ?? 0 })) : [];
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-6">
        <div className="neu-card p-6 md:p-8">
              ...
              {FMR_DIMENSIONS.map(dim => (
                <div key={dim.id}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {dim.variables.map(v => (
                      <NeuSelect key={v.key} label={`${v.label} (${(v.weight * 100).toFixed(0)}%)`} value={form[v.key]} options={v.options} onChange={set(v.key)} required />
                    ))}
                  </div>
                </div>
              ))}
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-[#EBEFF5]">
            <button onClick={() => setStep(s => Math.max(0, s - 1))} disabled={step === 0} className="neu-btn-outline flex items-center gap-2 px-5 py-2.5 text-sm disabled:opacity-30">
            ...
            {step < 2 ? (
              <button onClick={() => setStep(s => Math.min(2, s + 1))} className="neu-btn flex items-center gap-2 px-5 py-2.5 text-sm">
                Next <ChevronRight size={16} />
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={submitting} className="neu-btn flex items-center gap-2 px-6 py-2.5 text-sm">
                {submitting ? <span className="animate-spin border-2 border-white/30 border-t-white rounded-full w-4 h-4" /> : <Save size={16} />}
                Save Retailer
              </button>
            )}
          </div>
              ...
              {[
                ...
                <div key={String(label)} className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold px-2 py-0.5 rounded-full"
                    style={{ background: val === "Yes" ? "#22C55E22" : "#EF444422", color: val === "Yes" ? "#16a34a" : "#dc2626" }}>
                    {String(val)}
                  </span>
                </div>
              ))}
