    ...
    <div className="fixed top-0 right-0 h-screen w-[420px] z-50 flex flex-col overflow-hidden"
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1.5">
              <span className="flex items-center gap-1 text-xs text-slate-light"><MapPin size={11} /> {retailer.city}</span>
              <span className="flex items-center gap-1 text-xs text-slate-light"><Globe size={11} /> {retailer.zone}</span>
              {retailer.timestamp && <span className="text-xs text-slate-light">{formatDate(retailer.timestamp)}</span>}
              <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: `${color}22`, color }}>{category}</span>
            </div>
          </div>
          <button onClick={onClose} className="ml-3 flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center hover:bg-[#EBEFF5] transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: "Final Score", value: `${(pct * 100).toFixed(1)}%`, color },
            { label: "Band", value: band, color: "#3A4750" },
            { label: "Category", value: category, color, small: true },
          ].map(({ label, value, color: c, small }) => (
            <div key={label} className="rounded-2xl p-3 text-center"
                 style={{ background: "#fff", boxShadow: "4px 4px 10px #c8cfd8, -4px -4px 10px #ffffff" }}>
              <p className={`${small ? "text-sm" : "text-2xl"} font-black leading-tight mt-1`} style={{ color: c }}>{value}</p>
              <p className="text-[10px] uppercase font-semibold tracking-wide text-slate-light mt-1">{label}</p>
            </div>
          ))}
        </div>

        <div className="rounded-2xl p-4 bg-white" style={{ boxShadow: "4px 4px 10px #c8cfd8, -4px -4px 10px #ffffff" }}>
              ...
              return (
                <div key={dim.id}>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <p className="text-[11px] font-bold ml-2 flex-shrink-0" style={{ color: barColor }}>{score.toFixed(2)}</p>
                  </div>
          ...
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={radarData} margin={{ top: 10, right: 30, bottom: 10, left: 30 }}>
                ...
                <PolarAngleAxis dataKey="dim" tick={{ fontSize: 9, fill: "#6B7A84" }} />
                <PolarRadiusAxis domain={[0, 5]} tick={false} axisLine={false} />
                <Radar name="Score" dataKey="score" stroke="#EF4444" fill="#EF4444" fillOpacity={0.25} strokeWidth={1.5} dot={{ r: 3, fill: "#EF4444" }} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        </div>

        <div className="rounded-2xl overflow-hidden bg-white" style={{ boxShadow: "4px 4px 10px #c8cfd8, -4px -4px 10px #ffffff" }}>
          <button
                    ...
                    >
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0"
                          style={{ background: (dimScore >= 3 ? "#2A7ADE" : color) + "22", color: dimScore >= 3 ? "#2A7ADE" : color }}>
                          {dimScore.toFixed(2)}
                        </span>
                      </div>
        ...
        <div className="rounded-2xl p-4 bg-white" style={{ boxShadow: "4px 4px 10px #c8cfd8, -4px -4px 10px #ffffff" }}>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={String(label)} className="rounded-xl p-2.5 text-center"
                style={{ background: val === "Yes" ? "#22C55E11" : "#F0F4F8", border: `1px solid ${val === "Yes" ? "#22C55E33" : "transparent"}` }}>
